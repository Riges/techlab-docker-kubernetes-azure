...
Afin de centraliser les ressources crées lors de ce Techlabs, nous créerons une ressource groupe du nom, par exemple, de **soat-techlab** dans la region **west europe** (comme tout le reste de nos ressources). On obtient alors la ressource suivante `az group create --name soat-techlab --location westeurope`. Ce qui nous donne :
