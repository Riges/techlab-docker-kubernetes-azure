...
Afin de centraliser les ressources créées lors de ce Techlabs, nous créerons une ressource groupe nommé, par exemple, de **soat-techlab** dans la region **west europe** (comme tout le reste de nos ressources). On obtient alors la commande suivante `az group create --name soat-techlab --location westeurope`. Ce qui nous donne :
